from affixes import drop_affixes
import re

# List of compound prefixes adapted from
# http://code.google.com/p/php-name-parser/
_compound_prefixes = ['vere','von','van','de','del','della','di','da','pietro','vanden','du', r'st\.','st','la','ter']
...
def split(name):
    ...
    ('Michael Joseph', 'de la Stephens')
    """
    # TODO: return affixes instead of just dropping them
    name = drop_affixes(name)
    i = name.find(', ')
    if i != -1:
